...
cloud_cb (const sensor_msgs::PointCloud2ConstPtr& input_cloud, const darknet_ros_msgs::BoundingBoxesConstPtr& input_detection)
{
  ...
  for(int i(0); i<num_boxes; i++){
   ...
   sor_noise.filter (*cloud_filtered_voxelgrid);
    ...
    sor_voxelgrid.setInputCloud (cloud_filtered_voxelgrid);
    sor_voxelgrid.setLeafSize (0.01, 0.01, 0.01); //size of the grid
    sor_voxelgrid.filter (*cloud_filtered_sor);
  ...
  seg.setInputCloud (cloud_filtered_voxelgrid);
  ...
  // Extract the inliers
  extract_plane.setInputCloud (cloud_filtered_voxelgrid);
  ...
  tf::Matrix3x3 rotation_matrix(normal_vector_abs.vector.x,0.0,cross_p.x,normal_vector_abs.vector.y,0.0,cross_p.y,0.0,1.0,0.0);
  ...
  if (object_name == "door"){
    ...
    *cloud_handle += *cloud_filtered_sor;
